...
BEGIN
  ...
  ELSE
    ...
    IF (OLD."resetRequested" AND NOT NEW."resetRequested") OR (OLD."resetCompleted" AND NOT NEW."resetCompleted") OR (NOT OLD."resetCompleted" AND NEW."resetCompleted" AND (NOT NEW."resetRequested" OR OLD.state<>'RUNNING')) THEN RAISE EXCEPTION 'Invalid connector sync reset marker'; END IF;
    IF NEW.state='RUNNING' THEN
      IF NEW."leaseUntil"<=CURRENT_TIMESTAMP THEN RAISE EXCEPTION 'Invalid connector sync lease: deadline is not in the future'; END IF;
      IF NEW."leaseUntil">CURRENT_TIMESTAMP+interval '60 seconds' THEN RAISE EXCEPTION 'Invalid connector sync lease: deadline exceeds limit'; END IF;
      IF NEW.attempts<>OLD.attempts+1 THEN RAISE EXCEPTION 'Invalid connector sync lease: claim generation mismatch'; END IF;
      IF OLD.state='RUNNING' AND OLD."leaseUntil">CURRENT_TIMESTAMP THEN RAISE EXCEPTION 'Invalid connector sync lease: previous claim remains active'; END IF;
    END IF;
