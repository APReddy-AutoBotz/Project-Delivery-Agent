...
| AC-CON-002 | `tests/jira-read-adapter.test.ts` checks configured visibility, rejects out-of-scope search results, rechecks direct issue lookup, and withholds cross-scope relations. `tests/ingestion-persistence.integration.test.ts` checks current project-reader authorization. | Partial. Add criterion-level evidence that a project unavailable to the integration identity is absent from discovery and cannot be synchronized or directly queried through the authorized runtime path. Current connector fixtures are synthetic. |
...
| AC-CON-004 | `tests/connector-runtime.integration.test.ts` verifies repeated authenticated webhook deliveries reuse the receipt/job and conflicting bodies are rejected; `tests/ingestion-persistence.integration.test.ts` verifies event replay reuses its receipt. | Partial. Add a durable observation/outcome count assertion and prove duplicate delivery causes no duplicate downstream obligation through the complete processing path. |
...
| AC-CON-008 | `tests/connector-runtime.integration.test.ts` covers a single-owner refresh lease, atomic completion and stale-operation fencing; token and retry classes are covered in `tests/connector-runtime-auth.test.ts` and `tests/jira-runtime.test.ts`. | Partial. The current lease contender check is sequential. Add a true concurrent refresh race, reload/restart verification of the newly encrypted token and revision, and a revoked-token failure with a redacted administrator action. |
