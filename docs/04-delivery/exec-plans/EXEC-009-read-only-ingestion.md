...
| AC-MNT-003 | `tests/connector.test.ts` exercises the shared synthetic identity/page/cursor/duplicate/permission/retry contract. `tests/jira-read-adapter.test.ts` rejects duplicate issue identities and verifies unknown-outcome redaction. `tests/jira-runtime-contract.test.ts` drives `JiraRuntimeService` through duplicate identities, bounded 429 advice and an unknown transport outcome; `tests/connector-runtime.integration.test.ts` confirms the durable scheduled job remains READY at attempt 2 after reset/read claims while the source cursor revision stays unchanged. Hosted architecture/contracts checks enforce the SDK boundary. [PR #71 exact-head Foundation #260 and Documentation #318](https://github.com/APReddy-AutoBotz/Project-Delivery-Agent/pull/71) and [post-merge Foundation #261 and Documentation #319](https://github.com/APReddy-AutoBotz/Project-Delivery-Agent/pull/71) passed after independent exact-candidate review. | Partial. Synthetic conformance and runtime implementation evidence pass; no live Jira integration identity/customer site is active under OD-013. Keep the Issue #7 acceptance checkbox open pending the full scope review; no accepted-story total changes. |
