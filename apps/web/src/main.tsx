...
function evidenceLocation() {
  ...
  return projectId &&
    uuid.test(projectId) &&
    ((assessmentId && !requestId && !scalarRequestId && uuid.test(assessmentId)) ||
      (requestId && !assessmentId && !scalarRequestId && uuid.test(requestId)) ||
      (scalarRequestId && !assessmentId && !requestId && uuid.test(scalarRequestId)))
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  // FR-EVD-009 / SEC-SECRET-001: consume discarded bodies on early exits, too.
  // Start the reader before throwing, but never delay a known denial/session
  // clear on body delivery. The recorder independently requires complete bytes.
...
function App() {
  ...
  const [linkedScalarRequest, setLinkedScalarRequest] = useState<string | null>(null);
  React.useEffect(() => {
    expireSession = () => {
            ...
            <>
              ...
              {retainedProject ? (
                <>
                  ...
                  />
                  <ScalarReconciliation
                    key={`scalar-reconciliation:${retainedProject.id}`}
                    projectId={retainedProject.id}
                    request={request}
                    initialRequestId={linkedScalarRequest}
                    visible={!project.isFetching && !project.isError}
                  />
                  <ProjectEvidence
